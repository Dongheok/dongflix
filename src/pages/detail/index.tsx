...
const Detail = () => {
    ...
    const DetailDataFunction = async () => {
        ...
        try {
            if (dataType === 'movie') {
                const response = await movieApi.movieDetail(parseIntId);
                result = response.data;
            } else {
                const response = await tvApi.tvDetail(parseIntId);
                result = response.data;
            }
        } catch (e) {
    ...
    return (
        <>
            ...
            {data.error !== '' ? (
                ...
                <Wrapper>
                    <Backdrop bgUrl={data.result.backdrop_path ? `https://image.tmdb.org/t/p/original${data.result.backdrop_path}` : '/dongflix/images/notVideo.jpg'} />
                    <Grid container className="detail">
                        <img src={data.result.poster_path ? `https://image.tmdb.org/t/p/original${data.result.poster_path}` : '/dongflix/images/notVideo.jpg'} alt="poster" className="poster" />
                        <Grid className="text">
                            <Grid container alignItems="center">
                                <Grid item>
                                            ...
                                            <>
                                                <span className="genres">
                                                    {data.result.genres.map((x: any, index: number): any => (
                                                        <em key={x.name} className={index + 1 === data.result.genres.length ? 'last' : ''}>
                                                            {x.name}
                                                            &nbsp;
                                    ...
                                    <a href={data.result?.homepage ? `${data.result.homepage}` : '/'} target="_blank" rel="noopener noreferrer" className="review">
                                        READ REVIEW
                                    </a>
                                    ...
                                    {data.result?.videos && (
                                        <Grid className={`youtube ${youtubeSw && 'on'}`}>
                                            ...
                                            <Grid className="iframe_wrap">
                                                <iframe
                                                    title="youtube"
                                                    src={data.result.videos.results[0]?.key && `https://www.youtube.com/embed/${data.result.videos.results[0].key}`}
